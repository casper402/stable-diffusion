...
unet_missing_keys, unet_unexpected_key = unet.load_state_dict(unet_state_dict, strict=False)
total_model_keys = len(unet.state_dict().keys())
num_loaded_keys = total_model_keys - len(unet_missing_keys)
print(f"Unet successfully loaded: {num_loaded_keys} keys.")

if unet_unexpected_key:
    print(f"Warning: Unexpected keys in UNet state dict: {unet_unexpected_key}")
...
controlnet_missing_keys, controlnet_unexpected_keys = controlnet.load_state_dict(unet_state_dict, strict=False)
total_model_keys = len(controlnet.state_dict().keys())
num_loaded_keys = total_model_keys - len(controlnet_missing_keys)
print(f"Controlnet successfully loaded: {num_loaded_keys} keys.")
...
dr_module.load_state_dict(torch.load(dr_weights_path, map_location=device))
dr_module.train()
